...
private:
	...
	double find_new_color(double a_color, int bit) {
		...
		if (bit == 8) {
			...
			//double new_color = (color & 1 << 7) + (color & 1 << 6) + (color & 1 << 5) + (color & 1 << 4) + (color & 1 << 3) + (color & 1 << 2) + (color & 1 << 1) + (color & 1 << 7) / 128;
...
public:
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				if (!gradient) {
					...
					pict[i][j] = (j * 1.0/(double(width)-1))*colorDepth;
					/*if (gamma != 0) {
						new_pixel = pow(new_pixel, gamma);
						pict[i][j] = new_pixel * colorDepth;
					}
	...
	void random_dithering(int bit) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				pict[i][j] = find_new_color(sum_between(pict[i][j]/colorDepth,rand()*1.0/(RAND_MAX-1)) * colorDepth, bit);
			}
		}
	...
	void floyd_dithering(int bit) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				//double a = sum_between(pict[i][j] / colorDepth, errors[i][j]/colorDepth) * colorDepth;
